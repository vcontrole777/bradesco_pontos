...
// ── Pending fbq queue ───────────────────────────────────────
// Events fired before loadMetaPixel (pixel ID fetched async from DB) are
// stored here and flushed after fbq("init") so the order is always correct:
// init → pending events → future events.
const pendingFbq: Array<unknown[]> = [];
...
export function loadMetaPixel(pixelId: string) {
  ...
  // Flush events that fired before the pixel was ready (e.g. CompleteRegistration
  // triggered before the async DB fetch for the pixel ID completed).
  pendingFbq.forEach((args) => window.fbq(...args));
  pendingFbq.length = 0;
...
export function trackLead(params?: Record<string, unknown>) {
  ...
  fbqCall("track", "Lead", payload, { eventID: eventId });
...
export function trackCompleteRegistration(params?: Record<string, unknown>) {
  ...
  fbqCall("track", "CompleteRegistration", payload, { eventID: eventId });
...
export function trackCustomEvent(eventName: string, params?: Record<string, unknown>) {
  ...
  fbqCall("trackCustom", eventName, payload, { eventID: eventId });
